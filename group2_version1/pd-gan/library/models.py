...
class Hardmap_Mask_Update(nn.Module):
    ...
    def __init__(self, batch_size, k_for_hard_map):
        """
    ...
    def forward(self, mask):
        ...
        for imask in range(self.batch_size):
...
class HardSPDNorm(nn.Module):
    ...
    def __init__(self, batch_size, channels, img_size, k_for_hard_map):
        """
        ...
        self.mask_update = Hardmap_Mask_Update(batch_size, k_for_hard_map=k_for_hard_map)
...
class SPD_Norm_Residual_Block(nn.Module):
    ...
    def __init__(self, batch_size, channels, img_size, k_for_hard_map):
        ...
        self.hsn1 = HardSPDNorm(batch_size, channels, img_size, k_for_hard_map=k_for_hard_map)
        self.relu1 = nn.ReLU(inplace=False)
        self.conv1 = nn.Conv2d(channels, channels, 3, 1, 1)

        self.hsn2 = HardSPDNorm(batch_size, channels, img_size, k_for_hard_map=k_for_hard_map)
...
class PDGAN(nn.Module):
    ...
    def __init__(self, batch_size, first_channel_size, img_size, k_for_hard_map, noise_size):
        """
        ...
        self.latent_vector_size = int(self.batch_size * self.first_channel_size * self.img_size * self.img_size / 256)
        ...
        self.sn1 = SPD_Norm_Residual_Block(batch_size=self.batch_size, channels=self.first_norm_channel_size,
                                           img_size=self.first_norm_img_size, k_for_hard_map=k_for_hard_map)

        self.sn2 = SPD_Norm_Residual_Block(batch_size=self.batch_size, channels=self.second_norm_channel_size,
                                           img_size=self.second_norm_img_size, k_for_hard_map=k_for_hard_map)

        self.sn3 = SPD_Norm_Residual_Block(batch_size=self.batch_size, channels=self.third_norm_channel_size,
                                           img_size=self.third_norm_img_size, k_for_hard_map=k_for_hard_map)

        self.sn4 = SPD_Norm_Residual_Block(batch_size=self.batch_size, channels=self.last_norm_channel_size,
    ...
    def forward(self, img, mask, noise):
        ...
        noise = torch.reshape(noise, (1, self.noise_size * self.noise_size))

        # First layer is a fully connected layer
        out = self.fc(noise)

        # Reshape 1D vector output to block output (channels=24,height=16,width=16)
        out = torch.reshape(out, (self.batch_size, self.first_norm_channel_size,
